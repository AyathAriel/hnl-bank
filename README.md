...
| **Deploy** | ⚙️ Preparado | [`render.yaml`](render.yaml) (Render Blueprint) con los 4 servicios, discos persistentes para TigerBeetle/Postgres y variables de entorno. El Dockerfile del frontend soporta `VITE_API_URL` como build arg para plataformas donde los servicios no comparten red por nombre como en docker-compose (ej. frontend en Vercel + backend en Render). **No se ejecutó el deploy real** porque requiere una cuenta en la plataforma elegida — avísame si quieres que lo haga y con qué proveedor. |
| **Testing** | ✅ | 14 tests unitarios en el backend: conversión de dinero (`internal/ledger`, incluye un test de ida-y-vuelta centavos↔decimal para evitar errores de redondeo), hashing de contraseñas y generación/validación de JWT (`internal/auth`), y el middleware de autenticación completo con `httptest` (rechaza sin header, token inválido, token revocado; acepta uno válido y propaga el usuario al contexto). `cd backend && go test ./... -v`. |
| **CI/CD** | ✅ | [`.github/workflows/ci.yml`](.github/workflows/ci.yml): en cada push/PR a `main` corre `go vet` + `go build` + `go test` del backend, `npm run build` del frontend, y valida que las 3 imágenes Docker (backend, frontend, tigerbeetle) construyan sin errores. |
| **Responsive** | ✅ | Verificado explícitamente en 375px (móvil), 768px (tablet) y desktop en las 5 vistas. Se corrigieron 2 problemas reales encontrados en esta pasada: el texto de "cuenta origen → cuenta destino" se cortaba en 3 líneas en el historial (ahora se oculta en pantallas muy chicas, queda solo tipo/monto/fecha) y el widget de chat dejaba un bloque vacío enorme en móvil (altura ajustada por breakpoint). |
